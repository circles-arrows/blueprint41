...
namespace Blueprint41.GremlinUnitTest
{
    internal class GremlinStore : DatastoreModel<GremlinStore>
    {
        ...
        public void Initialize()
        {
            ...
            .AddProperty("TagLine", typeof(string), false)
            .AddProperty("ReleaseDate", typeof(DateTime));

            Entities.New("Actor", Entities["Base"])
    ...
    internal class TestGremlinDataStore : GremlinBase
    {
        ...
        public void GenerateGremlinEntityFiles()
        {
            ...
            Assert.True(File.Exists(Path.Combine(entitiesFolder, "Actor.cs")));

            // Nodes
            Assert.True(File.Exists(Path.Combine(nodesFolder, "FilmNode.cs")));
            Assert.True(File.Exists(Path.Combine(nodesFolder, "ActorNode.cs")));
