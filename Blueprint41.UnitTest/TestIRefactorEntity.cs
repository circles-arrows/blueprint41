...
{
    internal class TestIRefactorEntity
    {
        ...
        private class DataModelWithStaticDataKeyDuplicate : DatastoreModel<DataModelWithStaticDataKeyDuplicate>
        {
            ...
            public void Initialize()
            {
                ...
                Helper.AssertThrows<PersistenceException>(() =>
                {
                    Entities["ContactStatus"].Refactor.CreateNode(new { Uid = "3", Name = "Unknown", OrderBy = "11" });
                }, "A static entity with the same key already exists.");
            }
        }
        ...
        private class DataModelStaticDataWithDeleteNode : DatastoreModel<DataModelStaticDataWithDeleteNode>
        {
            ...
            public void DeleteNode()
            {
                ...
                DynamicEntity deletedNode = Entities["ContactStatus"].StaticData.SingleOrDefault(x => x.GetDynamicEntityValues()["Uid"].ToString() == "1");
                Assert.IsNull(deletedNode);

                Helper.AssertThrows<ArgumentOutOfRangeException>(() =>
                {
                    Entities["ContactStatus"].Refactor.DeleteNode("1");
                }, "Specified argument was out of the range of valid values." + Environment.NewLine + "Parameter name: Only statically created data (via the upgrade script) can be deleted here.");

                Helper.AssertThrows<InvalidCastException>(() =>
                {
                    Entities["ContactStatus"].Refactor.DeleteNode(5);

                }, "The key for entity 'ContactStatus' is of type 'String', but the supplied key is of type 'Int32'.");

                Helper.AssertThrows<ArgumentNullException>(() =>
                {
                    Entities["ContactStatus"].Refactor.DeleteNode(null);
                });
            }
        }
        ...
        // TODO: IRefactorEntity ApplyConstraints
        ...
        private class DatastoreEntityBase : DatastoreModel<DatastoreEntityBase>
        {
            protected override void SubscribeEventHandlers()
            {
            ...
            public void Initialize()
            {
                ...
                Entities.New("BaseTwoWithParent", Entities["BaseOne"]);

                Entities.New("ChildTwo", Entities["BaseOne"])
                   .AddProperty("Name", typeof(string), false);
            }
            ...
            public void Update()
            {
                ...
                Entities["ChildTwo"].Refactor.ChangeInheritance(Entities["BaseTwoWithParent"]);
                Assert.AreEqual(Entities["ChildTwo"].Inherits.Name, "BaseTwoWithParent");
            }
        }
        ...
        {
            DatastoreModel model = new DatastoreEntityBase();
            model.Execute(false);
        }
        ...
        private class DatastoreEntityCopyValue : DatastoreModel<DatastoreEntityCopyValue>
        {
            ...
            public void Update()
            {
                ...
                }, "You cannot copy (potentially dynamic) data to a static data node.");

                Helper.AssertThrows<NotSupportedException>(() =>
                {
                    Entities["Account"].Refactor.CopyValue("Name", "NonExistentProperty");

                }, "The target property 'NonExistentProperty' was not found on entity 'Account', you cannot copy data outside of the entity with this refactor action.");

                Helper.AssertThrows<NotSupportedException>(() =>
                {
                    Entities["Account"].Refactor.CopyValue("NonExistentProperty", "Name");

                }, "The source property 'NonExistentProperty' was not found on entity 'Account', you cannot copy data outside of the entity with this refactor action.");
            }
        }
        ...
        public void IRefactorCopyValue()
        {
            ...
            // TODO: Moved to unit test that excute test for graph db's
        ...
        //        }, "The field 'Label' was not present on entity 'Account'.");
        ...
        private class DatastoreEntityFunctionalId : DatastoreModel<DatastoreEntityFunctionalId>
        {
            protected override void SubscribeEventHandlers()
            {
            ...
            [Version(0, 0, 2)]
            public void ChangeFunctionalId()
            {
            ...
            [Version(0, 0, 3)]
            public void WithBaseFunctionalId()
            {
                FunctionalId account = FunctionalIds.New("InheritedAccount", "IA_", IdFormat.Hash);

                Entities.New("BaseAccount", account)
                .AddProperty("Uid", typeof(string), false, IndexType.Unique)
                .SetKey("Uid", true);

                Entities.New("InheritedFunctionalId", Entities["BaseAccount"])
                  .AddProperty("Name", typeof(string), false)
                  .SetFullTextProperty("Name");

                Helper.AssertThrows<InvalidOperationException>(delegate ()
                {
                    FunctionalId toChangeFunctionalId = FunctionalIds.New("ChangeFunctionalId", "CF_", IdFormat.Hash);
                    Entities["InheritedFunctionalId"].Refactor.SetFunctionalId(toChangeFunctionalId);

                }, "The entity 'InheritedFunctionalId' already inherited a functional id 'IA_ (InheritedAccount)', you cannot assign another one.");
            }

            [Version(0, 0, 4)]
            public void ResetFunctionalId()
            {
                FunctionalId account = FunctionalIds.New("ResetAccount", "RA_", IdFormat.Hash);                

                Entities.New("ResetFunctionalId", account)
                  .AddProperty("Name", typeof(string), false)
                  .SetFullTextProperty("Name");

                Entities["ResetFunctionalId"].Refactor.ResetFunctionalId();

                Assert.AreEqual(Entities["ResetFunctionalId"].FunctionalId, FunctionalIds.Default);

                Helper.AssertThrows<InvalidOperationException>(delegate ()
                {
                    Entities["InheritedFunctionalId"].Refactor.ResetFunctionalId();

                }, "The entity 'InheritedFunctionalId' does not have a functional id, but inherited it 'IA_ (InheritedAccount)', you cannot assign another one.");
            }
        }

        [Test]
        public void IRefactorEntityFunctionalId()
        {
            DatastoreModel model = new DatastoreEntityFunctionalId();
            model.Execute(false);
