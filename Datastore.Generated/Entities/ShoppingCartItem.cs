...
{
	public interface IShoppingCartItemOriginalData : ISchemaBaseOriginalData
	{
		int Quantity { get; }
	...
	public partial class ShoppingCartItem : OGM<ShoppingCartItem, ShoppingCartItem.ShoppingCartItemData, System.String>, ISchemaBase, INeo4jBase, IShoppingCartItemOriginalData
	{
		...
		protected static void RegisterQuery(string name, Func<IMatchQuery, q.ShoppingCartItemAlias, IWhereQuery> query)
		{
			...
			IMatchQuery matchQuery = Blueprint41.Transaction.CompiledQuery.Match(q.Node.ShoppingCartItem.Alias(out alias, "node"));
		...
		public class ShoppingCartItemEvents
		{
			...
			private EventHandler<ShoppingCartItem, EntityEventArgs> onAfterSave;
			public event EventHandler<ShoppingCartItem, EntityEventArgs> OnAfterSave
			{
				add
				{
					lock (this)
			...
			{
				EventHandler<ShoppingCartItem, EntityEventArgs> handler = onAfterSave;
				if (handler is not null)
					handler.Invoke((ShoppingCartItem)sender, args);
			}
			...
			public static class OnPropertyChange
			{
				...
				public static event EventHandler<ShoppingCartItem, PropertyEventArgs> OnDateCreated
				{
					...
					remove
					{
						...
						{
							onDateCreated -= value;
							if (onDateCreated is null && onDateCreatedIsRegistered)
							{
								Members.DateCreated.Events.OnChange -= onDateCreatedProxy;
