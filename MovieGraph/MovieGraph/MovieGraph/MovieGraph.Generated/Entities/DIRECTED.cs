...
namespace Domain.Data.Manipulation
{
    ...
    {
        private DIRECTED(string elementId, Person @in, Movie @out, Dictionary<string, object> properties)
        {
        ...
        public void Assign()
        {
            ...
            Assignment[] GetAssignments(q.DIRECTED_ALIAS alias)
            {
        ...
        public static List<DIRECTED> Where(Func<Alias, QueryCondition> expression)
        {
            var query = Transaction.CompiledQuery
                .Match(node.Person.Alias(out var inAlias).In.DIRECTED.Alias(out var relAlias).Out.Movie.Alias(out var outAlias))
        ...
        public static List<DIRECTED> Where(JsNotation<System.DateTime?> CreationDate = default, JsNotation<Person> InNode = default, JsNotation<Movie> OutNode = default)
        {
        ...
        internal static List<DIRECTED> Load(ICompiled query) => Load(query, null);
        internal static List<DIRECTED> Load(ICompiled query, params (string name, object value)[] arguments)
        {
            var context = query.GetExecutionContext();
            ...
            return results.Select(result => new DIRECTED(
                result.elementId,
                result.@in,
                result.@out,
                result.properties
        ...
        public static Relationship Relationship => ThreadSafe.LazyInit(ref _relationship, () => MovieGraph.Model.Datastore.Model.Relations["DIRECTED"]);
        private static Relationship _relationship = null;

        /// <summary>
        /// CRUD Specific alias for relationship: (Person)-[DIRECTED]->(Movie)
        /// </summary>
        public partial class Alias
        {
            internal Alias(q.DIRECTED_ALIAS relAlias, q.PersonAlias inAlias, q.MovieAlias outAlias)
            {
                _relAlias = relAlias;
                _inAlias = inAlias;
                _outAlias = outAlias;
            ...
            /// <summary>
            /// Person in-node: (Person)-[DIRECTED]->(Movie)
            /// </summary>
            /// <returns>
            ...
            /// <summary>
            /// Movie out-node: (Person)-[DIRECTED]->(Movie)
            /// </summary>
            /// <returns>
            ...
            private readonly q.DIRECTED_ALIAS _relAlias;
    ...
    {
        public static void Assign(this IEnumerable<DIRECTED> @this)
        {
            var query = Transaction.CompiledQuery
                .Match(node.Person.Alias(out var inAlias).In.DIRECTED.Alias(out var relAlias).Out.Movie.Alias(out var outAlias))
