...
namespace Domain.Data.Manipulation
{
    ...
    {
        private PRODUCED(string elementId, Person @in, Movie @out, Dictionary<string, object> properties)
        {
        ...
        public void Assign()
        {
            ...
            Assignment[] GetAssignments(q.PRODUCED_ALIAS alias)
            {
        ...
        public static List<PRODUCED> Where(Func<Alias, QueryCondition> expression)
        {
            var query = Transaction.CompiledQuery
                .Match(node.Person.Alias(out var inAlias).In.PRODUCED.Alias(out var relAlias).Out.Movie.Alias(out var outAlias))
        ...
        public static List<PRODUCED> Where(JsNotation<System.DateTime?> CreationDate = default, JsNotation<Person> InNode = default, JsNotation<Movie> OutNode = default)
        {
        ...
        internal static List<PRODUCED> Load(ICompiled query) => Load(query, null);
        internal static List<PRODUCED> Load(ICompiled query, params (string name, object value)[] arguments)
        {
            var context = query.GetExecutionContext();
            ...
            return results.Select(result => new PRODUCED(
                result.elementId,
                result.@in,
                result.@out,
                result.properties
        ...
        public static Relationship Relationship => ThreadSafe.LazyInit(ref _relationship, () => MovieGraph.Model.Datastore.Model.Relations["PRODUCED"]);
        private static Relationship _relationship = null;

        /// <summary>
        /// CRUD Specific alias for relationship: (Person)-[PRODUCED]->(Movie)
        /// </summary>
        public partial class Alias
        {
            internal Alias(q.PRODUCED_ALIAS relAlias, q.PersonAlias inAlias, q.MovieAlias outAlias)
            {
                _relAlias = relAlias;
                _inAlias = inAlias;
                _outAlias = outAlias;
            ...
            /// <summary>
            /// Person in-node: (Person)-[PRODUCED]->(Movie)
            /// </summary>
            /// <returns>
            ...
            /// <summary>
            /// Movie out-node: (Person)-[PRODUCED]->(Movie)
            /// </summary>
            /// <returns>
            ...
            private readonly q.PRODUCED_ALIAS _relAlias;
    ...
    {
        public static void Assign(this IEnumerable<PRODUCED> @this)
        {
            var query = Transaction.CompiledQuery
                .Match(node.Person.Alias(out var inAlias).In.PRODUCED.Alias(out var relAlias).Out.Movie.Alias(out var outAlias))
