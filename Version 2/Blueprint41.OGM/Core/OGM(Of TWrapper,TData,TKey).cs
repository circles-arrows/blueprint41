        ...
        where TWrapper : OGM<TWrapper, TKey>, new()
    {
        ...
        internal static OGM? Map(driver.NodeResult node, string cypher, Dictionary<string, object?>? parameters, persistence.NodeMapping mappingMode)
        {
            ...
            if (cypher is not null)
            {
                ...
                NodeEventArgs args = Entity.RaiseOnNodeLoaded(trans, loadingArgs, node.ElementId, node.Labels, properties);
            ...
            if (instance.PersistenceState == PersistenceState.HasUid || instance.PersistenceState == PersistenceState.Loaded)
            {
                ...
                instance.PersistenceState = (mappingMode == persistence.NodeMapping.AsWritableEntity) ? PersistenceState.Loaded : PersistenceState.OutOfScope;
            }
