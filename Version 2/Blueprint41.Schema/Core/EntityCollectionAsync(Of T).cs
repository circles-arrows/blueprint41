...
{
    public partial class EntityCollectionAsync<TEntity> : EntityCollectionAsyncBase<TEntity>
        where TEntity : class, OGM
    {
        public EntityCollectionAsync(OGM parent, Property property, EntityFlavor flavor, Func<TEntity, Task>? eagerLoadLogic = null)
        ...
        protected override Task<IEnumerable<CollectionItem<TEntity>>> GetItemsAsync(DateTime? from, DateTime? till)
        {
        ...
        protected override async Task<TEntity?> GetOriginalItemAsync(DateTime? moment)
        {
            if (ParentProperty?.PropertyType != PropertyType.Lookup)
                throw new NotSupportedException("You cannot use GetOriginalItem on a property thats not a lookup.");
        ...
        protected override Task AddItemAsync(TEntity item, DateTime? moment, Dictionary<string, object>? properties)
        {
            ...
            //return Add(item, true, properties);
        }
        ...
        public async Task<EntityCollectionAsync<TEntity>> LoadAsync()
        {
            if (IsLoaded)
                return this;
