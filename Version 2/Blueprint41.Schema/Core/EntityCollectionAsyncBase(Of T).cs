...
{
    public abstract class EntityCollectionAsyncBase<TEntity> : EntityCollectionAsyncBase, ICollection<TEntity>, ILookupHelperAsync<TEntity>, IInnerDataCol<TEntity>
        where TEntity : class, OGM
    {
        protected EntityCollectionAsyncBase(OGM parent, Property property, EntityFlavor flavor, Func<TEntity, Task>? eagerLoadLogicAsync) : base(parent, property)
        {
            EagerLoadLogicAsync = eagerLoadLogicAsync;
        }

        protected Action<TEntity>? EagerLoadLogic;
        protected Func<TEntity, Task>? EagerLoadLogicAsync;
        ...
        public async Task<IEnumerable<TEntity>> OriginalDataAsync() { await LazyLoadAsync(); return LoadedData.Select(item => item.Item); }
        ...
        protected virtual async Task LazySetAsync()
        {
            if (Parent.PersistenceState == PersistenceState.Persisted && Transaction != Transaction.RunningTransaction)
                throw new InvalidOperationException("This object was already flushed to the data-store.");
            else if (Parent.PersistenceState == PersistenceState.OutOfScope)
        ...
        protected abstract Task<TEntity?> GetOriginalItemAsync(DateTime? moment);

        protected abstract IEnumerable<CollectionItem<TEntity>> GetItems(DateTime? from, DateTime? till);
        protected abstract Task<IEnumerable<CollectionItem<TEntity>>> GetItemsAsync(DateTime? from, DateTime? till);
        protected abstract TEntity? GetItem(DateTime? moment);
        protected abstract Task<TEntity?> GetItemAsync(DateTime? moment);
        protected abstract void AddItem(TEntity item, DateTime? moment, Dictionary<string, object>? properties);
        protected abstract Task AddItemAsync(TEntity item, DateTime? moment, Dictionary<string, object>? properties);
        protected abstract void SetItem(TEntity? item, DateTime? moment, Dictionary<string, object>? properties);
        protected abstract Task SetItemAsync(TEntity? item, DateTime? moment, Dictionary<string, object>? properties);
        protected abstract bool IsNull(bool isUpdate);
        protected abstract void ClearLookup(DateTime? moment);
        protected abstract Task ClearLookupAsync(DateTime? moment);

        Task<TEntity?> ILookupHelperAsync<TEntity>.GetOriginalItemAsync(DateTime? moment)
        {
            return GetOriginalItemAsync(moment);
        }
        Task<TEntity?> ILookupHelperAsync<TEntity>.GetItemAsync(DateTime? moment)
        {
            return GetItemAsync(moment);
        }
        Task<IEnumerable<CollectionItem<TEntity>>> ILookupHelperAsync<TEntity>.GetItemsAsync(DateTime? from, DateTime? till)
        {
            return GetItemsAsync(from, till);
        }
        Task ILookupHelperAsync<TEntity>.AddItemAsync(TEntity item, DateTime? moment, Dictionary<string, object>? properties)
        {
            return AddItemAsync(item, moment, properties);
        }
        Task ILookupHelperAsync<TEntity>.SetItemAsync(TEntity? item, DateTime? moment, Dictionary<string, object>? properties)
        {
            return SetItemAsync(item, moment, properties);
        }
        bool ILookupHelperAsync<TEntity>.IsNull(bool isUpdate)
        {
            return IsNull(isUpdate);
        }
        Task ILookupHelperAsync<TEntity>.ClearLookupAsync(DateTime? moment)
        {
            return ClearLookupAsync(moment);
        }
    ...
    public interface IInnerDataColAsync
    {
    ...
    public interface IInnerDataColAsync<TEntity> : IInnerDataColAsync
    {
