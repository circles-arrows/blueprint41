...
{
    public partial class EntityTimeCollectionAsync<TEntity> : EntityCollectionAsyncBase<TEntity>
        where TEntity : class, OGM
    {
        public EntityTimeCollectionAsync(OGM parent, Property property, EntityFlavor flavor, Func<TEntity, Task>? eagerLoadLogicAsync = null) : base(parent, property, flavor, eagerLoadLogicAsync) { }
        ...
        protected override async Task<TEntity?> GetItemAsync(DateTime? moment)
        {
            if (ParentProperty?.PropertyType != PropertyType.Lookup)
                throw new NotSupportedException("You cannot use GetItem on a property thats not a lookup.");
        ...
        protected override async Task<IEnumerable<CollectionItem<TEntity>>> GetItemsAsync(DateTime? from, DateTime? till)
        {
            await LazyLoadAsync();
        ...
        protected async override Task<TEntity?> GetOriginalItemAsync(DateTime? moment)
        {
            if (ParentProperty?.PropertyType != PropertyType.Lookup)
                throw new NotSupportedException("You cannot use GetOriginalItem on a property thats not a lookup.");
        ...
        protected override Task AddItemAsync(TEntity item, DateTime? moment, Dictionary<string, object>? properties)
        {
        ...
        protected override async Task SetItemAsync(TEntity? item, DateTime? moment, Dictionary<string, object>? properties)
        {
            if (ParentProperty?.PropertyType != PropertyType.Lookup)
                throw new NotSupportedException("You cannot use SetItem on a property thats not a lookup.");
        ...
        protected override async Task ClearLookupAsync(DateTime? moment)
        {
            await LazySetAsync();
            ExecuteAction(ClearAction(moment));
        }
        ...
        public async Task<EntityTimeCollectionAsync<TEntity>> LoadAsync()
        {
            if (IsLoaded)
                return this;
