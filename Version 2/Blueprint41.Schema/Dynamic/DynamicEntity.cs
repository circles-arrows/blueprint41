...
namespace Blueprint41.Dynamic
{
    ...
    {
        internal DynamicEntity(Transaction? transaction, Entity entity, bool shouldExecute, object? initialize = null)
        {
        ...
        {
            if (Transaction is not null && Transaction.InTransaction)
            {
        ...
        public PersistenceState OriginalPersistenceState { get; set; }
        ...
        public static dynamic? Load(Transaction transaction, Entity entity, object? key)
        {
            if (key is null)
                return null;

            DynamicEntity? item = Lookup(transaction, entity, key);
            if (item is null)
                return null;
        ...
        public static dynamic? Lookup(Transaction transaction, Entity entity, object? key)
        {
            if (key is null)
                return null;
