...
namespace Blueprint41.Persistence
{
    internal class NodePersistenceProvider
    {
        ...
        public Task<List<T>> GetAllAsync<T>(Entity entity)
            where T : class, OGM
        {
            return LoadWhereAsync<T>(entity, string.Empty, null, 0, 0);
        }
        public Task<List<T>> GetAllAsync<T>(Entity entity, int page, int pageSize, bool ascending = true, params Property[] orderBy)
            where T : class, OGM
        {
            return LoadWhereAsync<T>(entity, string.Empty, null, page, pageSize, ascending, orderBy);
        }

        public async Task LoadAsync(OGM item, bool locked = false)
        {
            Entity entity = item.GetEntity();
            if (entity.Key is null)
                throw new InvalidOperationException("No key has been defined for this entity.");
            ...
            var result = await ((IStatementRunnerAsync)trans).RunAsync(args.Cypher, args.Parameters);

            driver.Record? record = await result.FirstOrDefaultAsync();
            if (record is null || record["node"] is null)
            {
                item.PersistenceState = PersistenceState.DoesntExist;
                return;
        ...
        public async Task DeleteAsync(OGM item)
        {
            Transaction trans = Transaction.RunningTransaction;
            Entity entity = item.GetEntity();
            ...
            var result = await ((IStatementRunnerAsync)trans).RunAsync(args.Cypher, args.Parameters);
            var counters = await result.StatisticsAsync();
            if (counters.NodesDeleted == 0)
                throw new DBConcurrencyException($"The {entity.Name} with {entity.Key.Name} '{item.GetKey()?.ToString() ?? "<NULL>"}' was changed or deleted by another process or thread.");
        ...
        public async Task ForceDeleteAsync(OGM item)
        {
            Transaction trans = Transaction.RunningTransaction;
            Entity entity = item.GetEntity();
        ...
        public async Task InsertAsync(OGM item)
        {
            Transaction trans = Transaction.RunningTransaction;
            Entity entity = item.GetEntity();
        ...
        public async Task UpdateAsync(OGM item)
        {
            ...
            driver.ResultCursor result = await ((IStatementRunnerAsync)trans).RunAsync(args.Cypher, args.Parameters);
            if (!(await GetStatisticsAsync(result)).ContainsUpdates)
                throw new DBConcurrencyException($"The {entity.Name} with {entity.Key.Name} '{item.GetKey()?.ToString() ?? "<NULL>"}' was changed or deleted by another process or thread.");
        ...
        public async Task<string> NextFunctionIDAsync(FunctionalId functionalId)
        {
            if (functionalId is null)
                throw new ArgumentNullException("functionalId");

            driver.ResultCursor results = await Transaction.RunAsync(NextFunctionalIdQuery(functionalId));
            driver.Record? result = await results.FirstOrDefaultAsync();
            
            return result?["key"]?.ToString()!;
        }

        public async Task<List<T>> LoadWhereAsync<T>(Entity entity, string conditions, IParameter[]? parameters, int page, int pageSize, bool ascending = true, params Property[] orderBy)
            where T : class, OGM
        {
            Transaction trans = Transaction.RunningTransaction;
            ...
            var result = await ((IStatementRunnerAsync)trans).RunAsync(args.Cypher, args.Parameters);
            return await LoadAsync<T>(entity, args, result, trans);
        }
        public async Task<List<T>> LoadWhereAsync<T>(Entity entity, ICompiledQuery query, IParameter[] parameters, int page = 0, int pageSize = 0, bool ascending = true, params Property[] orderBy)
            where T : class, OGM
        {
            Transaction trans = Transaction.RunningTransaction;
            ...
            var result = await Transaction.RunAsync(args.Cypher, args.Parameters);

            return await LoadAsync<T>(entity, args, result, trans);
        }

        internal async Task<List<T>> LoadAsync<T>(Entity entity, NodeEventArgs args, Persistence.ResultCursor result, Transaction trans)
            where T : class, OGM
        {
            if (entity.Key is null)
                throw new InvalidOperationException("No key has been defined for this entity.");
        ...
        internal async Task<List<T>> SearchAsync<T>(Entity entity, string text, Property[] fullTextProperties, int page = 0, int pageSize = 0, bool ascending = true, params Property[] orderBy)
            where T : class, OGM
        {
            if (entity.Key is null)
                throw new InvalidOperationException("No key has been defined for this entity.");
        ...
        public async Task<bool> RelationshipExistsAsync(EntityProperty foreignProperty, OGM item)
        {
            Entity entity = item.GetEntity();
            ...
            var result = await Transaction.RunAsync(match, parameters);
            return (await result.FirstOrDefaultAsync()) is not null;
        }

        private Task<driver.Counters> GetStatisticsAsync(driver.ResultCursor result)
        {
            return result.StatisticsAsync();
        }
