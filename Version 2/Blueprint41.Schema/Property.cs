    ...
    public abstract partial class Property : IRefactorProperty, IPropertyCondition, IPropertyEvents
    {
        ...
        void IRefactorProperty.Rename(string newName)
        {
            ...
            if (PropertyType == PropertyType.Attribute)
            {
                ...
                {
                    Parent.Parent.PersistenceProvider.Templates.RenameProperty(template =>
                    {
        ...
        private void RemoveIndexesAndContraints()
        {
            ...
            Parent.Parent.GetSchemaInfo().RemoveIndexesAndContraints(this);
        }
        ...
        private void MergeProperty(Property target, MergeAlgorithm mergeAlgorithm)
        {
            ...
            if (mergeAlgorithm == MergeAlgorithm.ThrowOnConflict)
            {
                foreach (var entity in Parent.GetConcreteClasses())
                {
                    long conflicts = Parent.Parent.PersistenceProvider.Templates.MergeProperty(template =>
                    {
                        template.From = this;
                        template.To = target;
                        template.Caller = entity; // ConcreteParent
            ...
            foreach (var entity in Parent.GetConcreteClasses())
            {
                Parent.Parent.PersistenceProvider.Templates.MergeProperty(template =>
                {
                    template.From = this;
                    template.To = target;
                    template.Caller = entity; // ConcreteParent
...
#pragma warning restore S2583 // Conditionally executed code should be reachable
        ...
        void IRefactorProperty.Convert(Type target, bool skipConversionLogic)
        {
            ...
            if (!skipConversionLogic)
            {
                ...
                if (chkScript != NO_SCRIPT && convScript != NO_SCRIPT)
                {
                    foreach (var entity in Parent.GetConcreteClasses())
                    {
                        Parent.Parent.PersistenceProvider.Templates.Convert(template =>
                        {
        ...
        void IRefactorProperty.Deprecate()
        {
            ...
            if (PropertyType == PropertyType.Attribute || Relationship is null)
            {
                ...
                foreach (var entity in Parent.GetConcreteClasses())
                {
                    Parent.Parent.PersistenceProvider.Templates.RemoveProperty(template =>
                    {
                        template.Name = Name;
                        template.Caller = entity; // ConcreteParent
                    }).RunBatched();
        ...
        void IRefactorProperty.MakeMandatory(object defaultValue)
        {
            ...
            if (PropertyType == PropertyType.Attribute && !(SystemReturnType is null))
            {
                ...
                foreach (var entity in Parent.GetConcreteClasses())
                {
                    Parent.Parent.PersistenceProvider.Templates.SetDefaultConstantValue(template =>
                    {
                        template.Caller = entity;
                        template.Property = this;
            ...
            else
            {
                ...
                Parent.Parent.PersistenceProvider.Templates.SetDefaultLookupValue(template =>
                {
        ...
        /// The property events
        /// </summary>
        public IPropertyEvents Events { get { return this; } }
