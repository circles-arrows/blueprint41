    ...
    public class SchemaInfo_Neo4jV5 : SchemaInfo_Neo4jV4
    {
        ...
        protected override void Initialize()
        {
            ...
            using (DatastoreModel.PersistenceProvider.NewTransaction(ReadWriteMode.ReadWrite))
            {
                ...
                Constraints       = LoadData("show constraints", record => NewConstraintInfo(record, DatastoreModel.PersistenceProvider)).Where(item => item.Entity is not null && item.Field is not null).ToArray();
                Indexes           = LoadData("show indexes", record => NewIndexInfo(record, DatastoreModel.PersistenceProvider)).Where(item => item.Entity is not null && item.Field is not null).ToArray();
        ...
        {
            bool first = true;
            string templateNumeric = "MATCH (node:{0}) WHERE toInteger(node.{1}) IS NOT NULL WITH toInteger(node.{1}) AS decoded RETURN CASE WHEN Max(decoded) IS NULL THEN 0 ELSE Max(decoded) END as MaxId";
            string templateHash = "MATCH (node:{0}) where node.{1} STARTS WITH '{2}' AND SIZE(node.{1}) = {3} CALL blueprint41.hashing.decode(replace(node.{1}, '{2}', '')) YIELD value as decoded RETURN CASE WHEN Max(decoded) IS NULL THEN 0 ELSE Max(decoded) END as MaxId";
        ...
        protected override ConstraintInfo NewConstraintInfo(IReadOnlyDictionary<string, object> rawRecord, PersistenceProvider persistenceProvider) => new ConstraintInfo_Neo4jV5(rawRecord, persistenceProvider);
        protected override IndexInfo NewIndexInfo(IReadOnlyDictionary<string, object> rawRecord, PersistenceProvider persistenceProvider) => new IndexInfo_Neo4jV5(rawRecord, persistenceProvider);
