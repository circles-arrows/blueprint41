...
namespace Datastore.Manipulation.Async
{
    ...
    public partial class ACTED_IN
    {
        ...
        public static Task<List<ACTED_IN>> WhereAsync(Func<Alias, QueryCondition> expression)
        {
            var query = Cypher
                .Match(node.Person.Alias(out var inAlias).In.ACTED_IN.Alias(out var relAlias).Out.Movie.Alias(out var outAlias))
        ...
        public static Task<List<ACTED_IN>> WhereAsync(JsNotation<System.DateTime?> CreationDate = default, JsNotation<Person> InNode = default, JsNotation<Movie> OutNode = default)
        {
        ...
        internal static async Task<List<ACTED_IN>> LoadAsync(ICompiled query, params (string name, object value)[] arguments)
        {
            var context = query.GetExecutionContext();
    ...
    {
        public static async Task AssignAsync(this IEnumerable<ACTED_IN> @this)
        {
            var query = Cypher
                .Match(node.Person.Alias(out var inAlias).In.ACTED_IN.Alias(out var relAlias).Out.Movie.Alias(out var outAlias))
