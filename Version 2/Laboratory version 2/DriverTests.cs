...
namespace Laboratory
{
    internal class DriverTests
    {
        ...
        public static async Task TestConnectivity()
        {
            ...
            await using (DriverSession session = driver.Session(config => { config.WithDatabase("unittest"); config.WithDefaultAccessMode(AccessMode.Write); }))
            {
            ...
            await using (DriverSession session = driver.Session(config => { config.WithDatabase("unittest"); config.WithDefaultAccessMode(AccessMode.Read); config.WithBookmarks(bookmark); }))
            {
